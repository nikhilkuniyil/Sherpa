...
$ sherpa -i
┌─────────────────────────────────────────────────────────────────────┐
│ Sherpa - Paper Implementation Coach                                 │
│                                                                     │
│ Your guide to implementing ML papers step by step.                  │
│                                                                     │
│ Mode: Tutorial mode - scaffolded learning with code skeletons       │
│ Commands: load, learn, explain, help | Type 'help' for all commands │
└─────────────────────────────────────────────────────────────────────┘
Claude API connected.

sherpa> load dpo_2023
Loaded: Direct Preference Optimization: Your Language Model is Secretly a Reward Model
Difficulty: intermediate | Time: 16-24 hours | Value: very_high

sherpa [dpo_2023]> learn
Before we write any code - let's think about the problem.

We have a language model and we want it to prefer "good" responses over "bad" ones.
The traditional approach (RLHF) trains a separate reward model, then uses RL.

DPO throws most of that away. Here's the question:
**How would YOU think about teaching a model to prefer one response over another
WITHOUT a separate reward model?**

[Think about the approach before we code...]

sherpa [dpo_2023]> The model already assigns probabilities to responses?

Exactly! The model already knows how likely it thinks each response is.
DPO's key insight is using that signal directly.

Here's your first code skeleton:

```python
class SimpleDPO:
    def __init__(self, model, ref_model, beta=0.1):
        # TODO 1: Store the policy model, reference model, and beta
        # Goal: Set up the two models - one trainable, one frozen as baseline
        pass
```

Complete TODO 1: Set up the two models - one trainable, one frozen as baseline
```

## Tutoring Modes

### Tutorial Mode (Default)
Scaffolded learning with code skeletons. Sherpa provides structure and hints while you fill in the implementation.

- Asked to reason about the approach before seeing code
- Code skeletons with TODOs to complete
- Feedback on each submission
- Checkpoint questions verify understanding

### Guided Mode
Full explanations with complete code. Use when you're stuck or want to understand concepts before implementing.
...
### Challenge Mode
Requirements only—you write from scratch. Tests your understanding after learning.

- Given requirements and constraints
- Write the complete implementation yourself
- Comprehensive feedback on your solution

### Debug Mode
Find and fix bugs in broken code. Teaches debugging skills and common pitfalls.
